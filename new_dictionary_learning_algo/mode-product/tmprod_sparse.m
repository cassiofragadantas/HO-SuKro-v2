function [S,iperm] = tmprod_sparse(T,U,mode,transpose,saveperm)
%TMPROD_SPARSE Variation of tmprod where the unfolded version of data 
%   tensor is transformed into sparse just before multiplying (considering 
%   that the tensor itself cannot be sparse).
%
%   Comments: doesn't work because 1) doesn't accelerate that much, 2)
%   after first product, the data is no longer sparse.
%
...
for i = 1:n
    ...
    switch transpose
        case 'T'
            S = reshape(full(U{i}.'*sparse(reshape(S,size(S,1),[]))),size_tens); % sparse, this is the difference wrt tmprod
        case 'H'
            S = reshape(full(U{i}'*sparse(reshape(S,size(S,1),[]))),size_tens);
        otherwise
            S = reshape(full(U{i}*sparse(reshape(S,size(S,1),[]))),size_tens);
    end
    if i < n
