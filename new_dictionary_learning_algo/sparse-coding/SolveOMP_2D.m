function [sols, y_est, iters, activationHist] = SolveOMP_2D(A, y, N, A_ip,normVec,tensDims,R, maxIters, lambdaStop, solFreq, verbose, OptTol)
%SolveOMP_tensor: based on SolveOMP function from SparseLab toolbox. It
%   supposes the dictionary A to be a 2D-SuKro dictionary.
%   The correlation calculation (A'*res) is replaced by a sum of smaller 
...
%    
    ...
    corr = A_ip{1,1}.'*reshape(res,tensDims)*A_ip{2,1};
...
%         corr = corr + modeprod3(reshape(res,tensDims),A_ip{1,p}.',A_ip{2,p}.',A_ip{3,p}.',tensDims);
        corr = corr + A_ip{1,p}.'*reshape(res,tensDims)*A_ip{2,p};
    ...
    [R_I, ~] = updateChol(R_I, n, N, A, explicitA, activeSet, newIndex,opts_tr, machPrec);
...
function [R, flag] = updateChol(R, n, N, A, explicitA, activeSet, newIndex,opts_tr, machPrec)
