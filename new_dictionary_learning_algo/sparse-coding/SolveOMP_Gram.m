function [sols, y_est, iters, activationHist] = SolveOMP_Gram(A, y, N, G, maxIters, lambdaStop, solFreq, verbose, OptTol)
% SolveOMP_Gram: Orthogonal Matching Pursuit
% Same as SolveOMP, but if the Gram matrix is provided, the inner products
% between the new atom and the current support is not recalculated at each
% iteration.
%
% Usage
...
%               no output (default)
%	OptTol      Error tolerance, compared directly w/ norm(res). Default 1e-5
% Outputs
...
while ~done
    ...
    if isempty(G) % use the pre-calculated correlations between atoms (if provided)
        corr_newAtom = [];
    else
        corr_newAtom = G(activeSet,newIndex);
    end
    
    [R_I, flag] = updateChol(R_I, n, N, A, explicitA, activeSet, newIndex, corr_newAtom);
    if flag==0
...
function [R, flag] = updateChol(R, n, N, A, explicitA, activeSet, newIndex, corr_newVec)
...
if isempty(activeSet)
    ...
    if (explicitA)
        if isempty(corr_newVec), corr_newVec = A(:,activeSet)'*A(:,newIndex); end
        p = linsolve(R,corr_newVec,opts_tr);
    else
